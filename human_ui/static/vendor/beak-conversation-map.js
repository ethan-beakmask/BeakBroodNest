/**
 * BeakConversationMap v0.1 -- 對話拓樸視覺化元件（基於 BeakGantt 架構）
 *
 * 主要功能：drawTraceMode(turns) 直式 git graph 因果鏈渲染
 * 保留 BeakGantt 原始 WBS+Gantt 功能作為底層基礎
 *
 * Usage:
 *   var g = BeakConversationMap.create('#el', {
 *       data: { tasks: [...], links: [...] },
...
var BeakConversationMapChart = (function() {
    ...
    P._init = function() {
        var r=this._container; r.innerHTML=''; r.classList.add('bk-cm');
        this._gridEl=_el('div','bk-cm-grid'); this._gridEl.style.width=this._opts.gridWidth+'px';
        r.appendChild(this._gridEl);
        this._splitterEl=_el('div','bk-cm-splitter'); r.appendChild(this._splitterEl); this._initSplitter();
        this._timelineEl=_el('div','bk-cm-timeline'); r.appendChild(this._timelineEl);
        this._scaleEl=_el('div','bk-cm-scale'); this._timelineEl.appendChild(this._scaleEl);
        this._rowsEl=_el('div','bk-cm-rows'); this._timelineEl.appendChild(this._rowsEl);
        this._depsEl=_svg('svg',{'class':'bk-cm-deps'}); this._rowsEl.appendChild(this._depsEl);
    ...
    P._syncH = function() {
        ...
        var tr=this._rowsEl.querySelectorAll('.bk-cm-row:not(.bk-drop-zone-timeline)');
    ...
    P._renderTimeline = function() {
        ...
        this._depsEl=_svg('svg',{'class':'bk-cm-deps',width:tw,height:this._visibleList.length*ROW_H+ROW_H});
        ...
        this._rowsEl.appendChild(_el('div','bk-cm-row bk-drop-zone-timeline',{style:'border-top:2px dashed #ccc;'}));
    };
    ...
    P._initLinkDrag = function(dot,src,ri) {
        ...
        dot.addEventListener('mousedown',function(e){
            ...
            var trs=self._rowsEl.querySelectorAll('.bk-cm-row:not(.bk-drop-zone-timeline)');
    ...
    P._renderDeps = function() {
        ...
        var cw=VM[this._viewMode].colWidth,rng=this._timeRange;
        var idx={},ry=[],trs=this._rowsEl.querySelectorAll('.bk-cm-row:not(.bk-drop-zone-timeline)'),cy=0;
    ...
    P.drawTraceMode = function(turns, onNodeClick) {
        ...
        // Attach resolved actor to each element (avoid mutating input)
        var items = sorted.map(function(t) {
            return { t: t, actor: _resolveActor(t) };
        });

        // ---- Step 3: column order ----
        // cc-main → col 0, agents by first appearance, human last
        var actorFirstIdx = {};
        for (var i = 0; i < items.length; i++) {
            var a = items[i].actor;
            if (actorFirstIdx[a] === undefined) actorFirstIdx[a] = i;
        }
        var actors = Object.keys(actorFirstIdx);
        ...
        var actorCol = {};
        for (var i = 0; i < actors.length; i++) actorCol[actors[i]] = i;

        // ---- Step 4: max tokens for opacity ----
        var maxTok = 1;
        for (var i = 0; i < items.length; i++) {
            var tok = (items[i].t.usage_input_tokens || 0) + (items[i].t.usage_output_tokens || 0);
            if (tok > maxTok) maxTok = tok;
        }

        // ---- Step 5: canvas dimensions ----
        var totalW = Math.max(actors.length * BRANCH_W + PAD_X * 2, 560);
        var totalH = items.length * SLOT_H + HEADER_H + 60;
        ...
        defs.appendChild(_mkArrow('cm-arrow-blue',  '#3b82f6'));
        defs.appendChild(_mkArrow('cm-arrow-teal',  '#0891b2'));
        defs.appendChild(_mkArrow('cm-arrow-gray',  '#9ca3af'));
        svg.insertBefore(defs, _g);
        ...
        function _isAgent(actor) { return /^cc-main:agent:/.test(actor); }

        // ---- Step 7: compute node centres ----
        var pos = [];  // pos[i] = {cx, cy, x, y}
        for (var i = 0; i < items.length; i++) {
            var col = actorCol[items[i].actor];
            if (col === undefined) col = 0;
            var cx = PAD_X + col * BRANCH_W + BRANCH_W / 2;
            var cy = HEADER_H + i * SLOT_H + SLOT_H / 2;
            pos.push({ cx: cx, cy: cy, x: cx - NODE_W/2, y: cy - NODE_H/2 });
        }

        // ---- Step 8: draw guide lines + headers ----
        for (var i = 0; i < actors.length; i++) {
            var ax = PAD_X + i * BRANCH_W + BRANCH_W / 2;
            var gl = document.createElementNS(svgNS, 'line');
            gl.setAttribute('x1',String(ax)); gl.setAttribute('y1',String(HEADER_H));
            gl.setAttribute('x2',String(ax)); gl.setAttribute('y2',String(totalH-20));
            gl.setAttribute('stroke','#e5e7eb'); gl.setAttribute('stroke-width','1');
            gl.setAttribute('stroke-dasharray','4 4');
            _g.appendChild(gl);

            var lbl = document.createElementNS(svgNS, 'text');
            lbl.setAttribute('x',String(ax)); lbl.setAttribute('y','28');
        ...
        // ---- Step 9: draw edges ----
        // Strategy: sequential within same actor + spawn/return between cc-main and agents
        // This works pre- and post-migration without relying on UUID lookups.

        var actorPrev = {};      // actor -> last item index in this actor
        var ccMainLast = -1;     // last cc-main item index
        var agentFirst = {};     // agent -> first item index (for spawn)
        var agentLast  = {};     // agent -> last item index (for return)

        // Pass 1: collect per-actor first/last
        for (var i = 0; i < items.length; i++) {
            var actor = items[i].actor;
            if (agentFirst[actor] === undefined && _isAgent(actor)) agentFirst[actor] = i;
            if (_isAgent(actor)) agentLast[actor] = i;
        }

        function _drawEdge(fromIdx, toIdx, stroke, width, dash, marker) {
            var fp = pos[fromIdx], tp = pos[toIdx];
            var x1 = fp.cx, y1 = fp.cy + NODE_H/2, x2 = tp.cx, y2 = tp.cy - NODE_H/2;
            var d;
            if (Math.abs(x1 - x2) < 3) {
                d = 'M'+x1+','+y1+' L'+x2+','+y2;
            } else {
                // bezier curve for cross-branch
                var ctrl = Math.min(40, Math.abs(y2 - y1) / 2);
                d = 'M'+x1+','+y1+' C'+x1+','+(y1+ctrl)+' '+x2+','+(y2-ctrl)+' '+x2+','+y2;
            }
            var path = document.createElementNS(svgNS, 'path');
            path.setAttribute('d', d); path.setAttribute('fill', 'none');
            path.setAttribute('stroke', stroke); path.setAttribute('stroke-width', String(width));
            if (dash) path.setAttribute('stroke-dasharray', dash);
            if (marker) path.setAttribute('marker-end', 'url(#' + marker + ')');
            _g.appendChild(path);
        }

        // Track which agent-return connections we've already drawn
        var agentReturnDone = {};

        for (var i = 0; i < items.length; i++) {
            var actor = items[i].actor;

            // Sequential within same actor (thin gray)
            if (actorPrev[actor] !== undefined) {
                _drawEdge(actorPrev[actor], i, '#d1d5db', 1.5, null, null);
            }

            // Spawn: first turn of an agent → connect from last cc-main before it
            if (_isAgent(actor) && agentFirst[actor] === i && ccMainLast >= 0) {
                _drawEdge(ccMainLast, i, '#3b82f6', 2, null, 'cm-arrow-blue');
            }

            // Return: first cc-main turn after each agent's last turn
            if (actor === 'cc-main') {
                for (var ag in agentLast) {
                    if (!agentReturnDone[ag] && agentLast[ag] < i) {
                        _drawEdge(agentLast[ag], i, '#3b82f6', 1.5, '6 3', 'cm-arrow-blue');
                        agentReturnDone[ag] = true;
                    }
                }
                ccMainLast = i;
            }
        ...
        // ---- Step 10: draw nodes ----
        for (var i = 0; i < items.length; i++) {
            var t = items[i].t, actor = items[i].actor, p = pos[i];

            var tok = (t.usage_input_tokens || 0) + (t.usage_output_tokens || 0);
            // opacity: min 0.55 (visible even with 0 tokens), max 1.0
            var opac = tok > 0 ? 0.55 + (tok / maxTok) * 0.45 : 0.55;
            var baseClr = _actorColor(actor);
            ...
            rect.setAttribute('fill', isSuper ? '#9ca3af' : _rgba(baseClr, opac));
            rect.setAttribute('stroke', isSuper ? '#6b7280' : baseClr);
            rect.setAttribute('stroke-width','1.5');
            nodeG.appendChild(rect);

            // Label text: prefer span_kind > role, append tool_name if present
            var labelText = t.span_kind || t.role || '?';
            if (t.tool_name) labelText += ':' + t.tool_name;
            if (labelText.length > 16) labelText = labelText.slice(0,15) + '…';

            var txt = document.createElementNS(svgNS, 'text');
            txt.setAttribute('x', String(p.cx)); txt.setAttribute('y', String(p.y + NODE_H/2 + 4));
            txt.setAttribute('text-anchor','middle'); txt.setAttribute('font-size','10');
            txt.setAttribute('fill', isSuper ? '#9ca3af' : '#fff');
            ...
            title.textContent = [
                'actor: ' + actor + (t.actor_id ? '' : ' (derived)'),
                'kind: '  + (t.span_kind || '-'),
                'role: '  + (t.role     || '-'),
                t.tool_name ? 'tool: ' + t.tool_name : null,
                'tokens: in=' + (t.usage_input_tokens || 0) + ' out=' + (t.usage_output_tokens || 0),
                t.superseded_by ? 'superseded → ' + t.superseded_by : null,
                'time: '  + (t.timestamp || ''),
            ].filter(Boolean).join('\n');
            nodeG.appendChild(title);

            _g.appendChild(nodeG);
        }
    };
