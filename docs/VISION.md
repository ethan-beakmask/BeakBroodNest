...
BeakNote 的因果鍊 + 時間軸 + `blocks` 關係 + lifecycle 狀態，天然能回答：
- **卡在哪** -- 哪些上游原子未完成
- **為什麼不開始** -- 追溯阻塞鍊的根節點
...
**與 Obsidian 的根本差異**：Obsidian 那坨 edge 把所有筆記等價呈現，活的死的全擠在一起，失焦。BeakNote 讓活的知識凸顯，死的知識退場但不消失。
...
BeakNote 的連結知道「A 導致了 B」「C 反駁了 A」。
